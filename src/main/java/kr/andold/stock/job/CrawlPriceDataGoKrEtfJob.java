...
@Slf4j
@Service
public class CrawlPriceDataGoKrEtfJob implements Job {
	...
	public STATUS call() throws Exception {
		log.debug("{} 주식시세수집ETF::CrawlPriceDataGoKrEtfJob::call()", Utility.indentStart());
		long started = System.currentTimeMillis();

		CrawlPriceDataGoKrEtfJob that = (CrawlPriceDataGoKrEtfJob) ApplicationContextProvider.getBean(CrawlPriceDataGoKrEtfJob.class);
		that.setMap(map);
		STATUS result = that.main();

		log.debug("{} 『#{}』 주식시세수집ETF::CrawlPriceDataGoKrEtfJob::call() - {}", Utility.indentEnd(), result, Utility.toStringPastTimeReadable(started));
		return result;
	}
	...
	public static void regist(ConcurrentLinkedDeque<Job> deque, String code, ZonedDateTime zdt) {
		...
		CrawlPriceDataGoKrEtfJob job = CrawlPriceDataGoKrEtfJob.builder().build();
	...
	private static boolean containsOrModify(Job job, String code, ZonedDateTime date) {
		if (!(job instanceof CrawlPriceDataGoKrEtfJob)) {
			return false;
		}

		CrawlPriceDataGoKrEtfJob previous = (CrawlPriceDataGoKrEtfJob) job;
	...
	protected STATUS main() {
		log.debug("{} 주식시세수집ETF::CrawlPriceDataGoKrEtfJob::main()", Utility.indentStart());
		long started = System.currentTimeMillis();
		...
		try {
			...
			log.debug("{} 『{}』 주식시세수집ETF::CrawlPriceDataGoKrEtfJob::main() - {}", Utility.indentEnd(), STATUS.SUCCESS, Utility.toStringPastTimeReadable(started));
			return STATUS.SUCCESS;
		} catch (Exception e) {
			log.error("{} Exception:: {}", Utility.indentMiddle(), e.getLocalizedMessage(), e);
		}

		log.debug("{} 『{}』 주식시세수집ETF::CrawlPriceDataGoKrEtfJob::main() - {}", Utility.indentEnd(), STATUS.EXCEPTION, Utility.toStringPastTimeReadable(started));
