...
public class SeibroTest {
	...
	public void testPriceCurrentCompanyStringDate() {
		Result<ParserResult> result = service.priceCompany("017860",
				Date.from(ZonedDateTime.now().minusYears(3).toInstant()));
		log.info("{}", result);
		assertEquals(result.getStatus(), STATUS.SUCCESS);
	}
	...
	public void testPriceCurrentCompanyStringDate00400() {
		Result<ParserResult> result = service.priceCompany("00400",
				Date.from(ZonedDateTime.of(1976, 4, 15, 0, 0, 0, 0, Utility.ZONE_ID_KST).toInstant()));
		log.info("{}", result);
		assertEquals(result.getStatus(), STATUS.SUCCESS);
	}
	...
	public void testPriceCurrentEtfStringDate() {
		Result<ParserResult> result = service.priceEtf("473590",
				Date.from(ZonedDateTime.now().minusDays(20).toInstant()));
		log.info("{}", result);
		assertEquals(result.getStatus(), STATUS.SUCCESS);
	}
	...
	public void testPriceStringDate() {
		Result<ParserResult> result = service.price("473590", Date.from(ZonedDateTime.now().minusDays(20).toInstant()));
		log.info("{}", result);
		assertEquals(result.getStatus(), STATUS.SUCCESS);
	}
	...
	public void testDividendAsCompany() {
		Result<ParserResult> result = service.dividendAsCompany("003310",
				Date.from(ZonedDateTime.of(2013, 6, 30, 0, 0, 0, 0, Utility.ZONE_ID_KST).toInstant()));
		log.info("{}", result);
		assertEquals(result.getStatus(), STATUS.SUCCESS);
	}
